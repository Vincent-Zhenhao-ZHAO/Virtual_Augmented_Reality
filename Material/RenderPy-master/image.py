...
class Image(object):
	...
	# This is useful for doing further processing on the image -> openCV
	def get_np_array(self):
		# Convert buffer to NumPy array
		buffer_np = np.frombuffer(self.buffer, dtype=np.uint8)  
		# Reshape the array to the correct dimensions
		buffer_np = buffer_np.reshape(-1, self.width * 4 + 1)  
		# Exclude the null bytes and reshape to the final dimensions
		return buffer_np[:, 1:].reshape(self.height, self.width, 4) 
